...
#if NET_4_5 || NETSTANDARD1_3
			public int ReadAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
			{
				AssertLocked();
 				return m_realStream.Read(buffer, offset, count);
 			}

 			public void WriteAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
 			{
 				AssertLocked(); 
